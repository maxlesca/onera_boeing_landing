...
run-report:  ## per-run distribution (which run is an outlier, on which channel): make run-report CONFIG=ils_aligned_cfc [SAVE=1]
	$(PYTHON) -m boeing_landing.data.run_report --config $(CFGPATH) $(if $(SAVE),--save)
